...
if [ -n "${DISABLE_HEALTH_CHECK+1}" ] && [ "${DISABLE_HEALTH_CHECK,,}" = "true" ]; then
	echo "Disabling proxy health check"
	cp /usr/src/health_check/disable_health_check.conf /etc/nginx/conf.d/health_check.conf
else
	echo "Enabling proxy health check"
	cp /usr/src/health_check/proxy_health_check.conf /etc/nginx/conf.d/health_check.conf
fi
